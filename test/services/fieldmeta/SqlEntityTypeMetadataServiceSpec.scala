...
class SqlEntityTypeMetadataServiceSpec extends IntegrationTestRunner {
  ...
  "EntityTypeMetadataService" should {
    ...
    "get entity type" in new DBTestApp("entity-type-metadata-fixtures.sql") {
      val ds = await(service.get(EntityType.DocumentaryUnitDescription))
    ...
    "save entity type" in new DBTestApp("entity-type-metadata-fixtures.sql") {
      val ds = await(service.save(EntityType.RepositoryDescription, EntityTypeMetadataInfo(
    ...
    "locate fields by entity type" in new DBTestApp("entity-type-metadata-fixtures.sql") {
      val ds = await(service.listFields(Some(EntityType.RepositoryDescription)))
      ds.size must_== 1
    }

    "get fields" in new DBTestApp("entity-type-metadata-fixtures.sql") {
      val ds = await(service.getField(EntityType.DocumentaryUnitDescription, "locationOfOriginals"))
    ...
    "save fields" in new DBTestApp("entity-type-metadata-fixtures.sql") {
      val ds = await(service.saveField(EntityType.RepositoryDescription, "new", FieldMetadataInfo(
        "New Field",
    ...
    "save existing fields" in new DBTestApp("entity-type-metadata-fixtures.sql") {
      val ds = await(service.saveField(EntityType.RepositoryDescription, "history", FieldMetadataInfo(
        "Updated Field",
    ...
    "save fields with a variety of different array values" in new DBTestApp("entity-type-metadata-fixtures.sql") {
      ...
      val ds = await(service.saveField(EntityType.RepositoryDescription, "new", info))
      ds.seeAlso must_== Seq("New Field See Other", "https://example.com/new-field")
      val ds2 = await(service.saveField(EntityType.RepositoryDescription, "new2", info.copy(seeAlso = Seq())))
      ds2.seeAlso must_== Seq()
    }

    "delete fields" in new DBTestApp("entity-type-metadata-fixtures.sql") {
      await(service.deleteField(EntityType.RepositoryDescription, "history")) must_== true
      await(service.listFields()).size must_== 1
      await(service.listFields(Some(EntityType.RepositoryDescription))).size must_== 0
    }
  }
}
