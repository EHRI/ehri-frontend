...
import services.rest.{CypherIdGenerator, ItemNotFound, ValidationError}
 ...
 */
class RestApiSpec extends IntegrationTestRunner {
  sequential
   ...
   */
  case class TestResource(id: String, data: JsObject) extends services.WithId
  object TestResource {
    implicit object Resource extends services.Resource[TestResource] {
  ...
  "Generic entity operations" should {
    ...
    "page items" in new ITestApp {
      ...
      r.page mustEqual 1
      r.limit mustEqual services.rest.Constants.DEFAULT_LIST_LIMIT
      r.total mustEqual 5
    }
