...
class DAOSpec extends helpers.Neo4jRunnerSpec(classOf[DAOSpec]) {
  ...
  "EntityDAO" should {
    "get an item by id" in new FakeApp {
      await(EntityDAO[UserProfile](entityType, Some(userProfile)).get(userProfile.id))
    }
    ...
    "update an item by id" in new FakeApp {
      val user = UserProfileF(id = None, identifier = "foobar", name = "Foobar")
      val entity = await(EntityDAO[UserProfile](entityType, Some(userProfile)).create(user))
    ...
    "error when fetching a non-existing item" in new FakeApp {
      try {
        await(EntityDAO[UserProfile](entityType, Some(userProfile)).get("blibidyblob"))
    ...
    "delete an item by id" in new FakeApp {
      ...
      val entity = await(EntityDAO[UserProfile](entityType, Some(userProfile)).create(user))
      await(EntityDAO(entityType, Some(userProfile)).delete(entity.id))
    }

    "page items" in new FakeApp {
      val r = await(EntityDAO[UserProfile](entityType, Some(userProfile)).page(PageParams()))
    ...
    "count items" in new FakeApp {
      var r = await(EntityDAO(entityType, Some(userProfile)).count(PageParams()))
      r mustEqual 5L
    }
  ...
  "PermissionDAO" should {
    ...
    "be able to set a user's permissions" in new FakeApp {
      ...
      val newperms = await(PermissionDAO(Some(userProfile)).set(user, data))
    ...
    "be able to set a user's permissions within a scope" in new FakeApp {
      ...
      val newItemPerms = await(PermissionDAO(Some(userProfile)).getItem(user, ContentTypes.DocumentaryUnit, "c1"))
    ...
    "be able to set a user's permissions for an item" in new FakeApp {
      ...
      val newItemPerms = await(PermissionDAO(Some(userProfile)).setItem(user, ContentTypes.DocumentaryUnit, "c1", data))
      newItemPerms.get(PermissionType.Update) must beSome
      newItemPerms.get(PermissionType.Delete) must beSome
    }
  }
  ...
  "VisibilityDAO" should {
    "set visibility correctly" in new FakeApp {
      ...
      val c1a = await(EntityDAO[DocumentaryUnit](EntityType.DocumentaryUnit, Some(userProfile)).get("c1"))
      ...
      val c1b = await(EntityDAO[DocumentaryUnit](EntityType.DocumentaryUnit, Some(userProfile)).get("c1"))
