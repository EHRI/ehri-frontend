...
class EntityViewsSpec extends Neo4jRunnerSpec(classOf[EntityViewsSpec]) {
  ...
  "UserProfile views" should {
    ...
    "reliably set permissions" in new FakeApp {
      ...
      val cr = route(fakeLoggedInHtmlRequest(privilegedUser, POST,
        controllers.users.routes.UserProfiles.permissionsPost(subjectUser.id).url).withHeaders(formPostHeaders.toSeq: _*), testData).get
      status(cr) must equalTo(SEE_OTHER)
    ...
    "link to other privileged actions when logged in" in new FakeApp {
      ...
      contentAsString(show) must contain(controllers.users.routes.Groups.membership(EntityType.UserProfile, id).url)
    }
    ...
    "allow adding users to groups" in new FakeApp {
      ...
      val add = route(fakeLoggedInHtmlRequest(privilegedUser, POST,
        controllers.users.routes.Groups.addMemberPost("niod", EntityType.UserProfile, id).url)
    ...
    "allow removing users from groups" in new FakeApp {
      ...
      val rem = route(fakeLoggedInHtmlRequest(privilegedUser, POST,
        controllers.users.routes.Groups.removeMemberPost("kcl", EntityType.UserProfile, id).url)
          .withFormUrlEncodedBody()).get
  ...
  "Group views" should {
    ...
    "detail when logged in should link to other privileged actions" in new FakeApp {
      val show = route(fakeLoggedInHtmlRequest(privilegedUser, GET, controllers.users.routes.Groups.get(id).url)).get
      status(show) must equalTo(OK)
      contentAsString(show) must contain(controllers.users.routes.Groups.update(id).url)
      contentAsString(show) must contain(controllers.users.routes.Groups.delete(id).url)
      contentAsString(show) must contain(controllers.users.routes.Groups.permissions(id).url)
      contentAsString(show) must contain(controllers.users.routes.Groups.grantList(id).url)
      contentAsString(show) must contain(controllers.users.routes.Groups.membership(EntityType.Group, id).url)
      contentAsString(show) must contain(controllers.users.routes.Groups.list().url)
    }

    "allow adding groups to groups" in new FakeApp {
      // Add KCL to Admin
      val add = route(fakeLoggedInHtmlRequest(privilegedUser, POST,
        controllers.users.routes.Groups.addMemberPost("admin", EntityType.Group, id).url)
          .withFormUrlEncodedBody()).get
      status(add) must equalTo(SEE_OTHER)
    ...
    "allow removing groups from groups" in new FakeApp {
      ...
      val rem = route(fakeLoggedInHtmlRequest(privilegedUser, POST,
        controllers.users.routes.Groups.removeMemberPost("admin", EntityType.Group, "niod").url)
          .withFormUrlEncodedBody()).get
      status(rem) must equalTo(SEE_OTHER)
