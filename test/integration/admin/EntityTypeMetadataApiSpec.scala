...
class EntityTypeMetadataApiSpec extends IntegrationTestRunner with ResourceUtils {
  ...
  "EntityTypeMetadataApi" should {
    ...
    "get entity type" in new DBTestApp("entity-type-metadata-fixtures.sql") {
      val r = FakeRequest(entityTypeMetadataRoutes.get(EntityType.RepositoryDescription))
        .withUser(privilegedUser)
        .call()
      status(r) must_== OK
    ...
    "save entity type metadata" in new DBTestApp("entity-type-metadata-fixtures.sql") {
      ...
      val r = FakeRequest(entityTypeMetadataRoutes.save(EntityType.ConceptDescription))
        .withUser(privilegedUser)
        .callWith(data)
      status(r) must_== OK
      contentAsJson(r).asOpt[EntityTypeMetadata] must beSome.which { c: EntityTypeMetadata =>
    ...
    "get field metadata with no data" in new DBTestApp("entity-type-metadata-fixtures.sql") {
      val r = FakeRequest(entityTypeMetadataRoutes.getField(EntityType.RepositoryDescription, "no-such-field"))
        .withUser(privilegedUser)
        .call()
    ...
    "get field metadata" in new DBTestApp("entity-type-metadata-fixtures.sql") {
      val r = FakeRequest(entityTypeMetadataRoutes.getField(EntityType.RepositoryDescription, Isdiah.HISTORY))
        .withUser(privilegedUser)
        .call()
      status(r) must_== OK
    ...
    "create field metadata" in new DBTestApp("entity-type-metadata-fixtures.sql") {
      ...
      val r = FakeRequest(entityTypeMetadataRoutes.saveField(EntityType.RepositoryDescription, Isdiah.GEOCULTURAL_CONTEXT))
        .withUser(privilegedUser)
        .callWith(data)
    ...
    "update field metadata" in new DBTestApp("entity-type-metadata-fixtures.sql") {
      ...
      val r = FakeRequest(entityTypeMetadataRoutes.saveField(EntityType.RepositoryDescription, Isdiah.HISTORY))
        .withUser(privilegedUser)
        .callWith(data)
      status(r) must_== OK
      contentAsJson(r).asOpt[FieldMetadata] must beSome.which { c: FieldMetadata =>
    ...
    "delete field metadata" in new DBTestApp("entity-type-metadata-fixtures.sql") {
      val r = FakeRequest(entityTypeMetadataRoutes.deleteField(EntityType.RepositoryDescription, Isdiah.HISTORY))
        .withUser(privilegedUser)
        .call()
