...
import backend.{BackendContentType, Readable, BackendWriteable}
...
trait Create[F <: Model with Persistable, MT <: MetaModel[F]] extends Generic {
  ...
  protected def CreateItemAction(form: Form[F], pf: Request[_] => Map[String,Seq[String]] = _ => Map.empty)(implicit fmt: BackendWriteable[F], rd: Readable[MT], ct: BackendContentType[MT]) =
      ...
      def transform[A](request: WithUserRequest[A]): Future[CreateRequest[A]] = {
        ...
        form.bindFromRequest.fold(
          ...
          doc => {
            ...
            userBackend.create(doc, accessors, params = pf(request), logMsg = getLogMessage).map { item =>
