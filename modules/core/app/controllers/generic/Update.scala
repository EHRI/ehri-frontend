...
import backend.{Writable, ContentType}
...
trait Update[F <: Model with Persistable, MT <: MetaModel[F]] extends Generic {
  ...
  protected def EditAction(itemId: String)(implicit ct: ContentType[MT]) =
    WithItemPermissionAction(itemId, PermissionType.Update)

  protected def UpdateAction(id: String, form: Form[F], transformer: F => F = identity)(implicit ct: ContentType[MT], wd: Writable[F]) =
