...
case class Search @Inject() (implicit cache: CacheApi, app: play.api.Application, ws: WSClient) extends SearchDAO

/**
 * Resolve search hits to DB items by the GID field
 */
case class GidSearchResolver @Inject ()(implicit cache: CacheApi, app: play.api.Application, ws: WSClient) extends RestDAO with SearchDAO with SearchItemResolver {
 ...
 */
case class IdSearchResolver @Inject ()(implicit cache: CacheApi, app: play.api.Application, ws: WSClient) extends RestDAO with SearchDAO with SearchItemResolver {
  def resolve[MT](docs: Seq[SearchHit])(implicit apiUser: ApiUser,  rd: Readable[MT]): Future[Seq[MT]] =
