...
object EventStoreMediator {
  ...
  def sseEvent: PartialFunction[EventForwarder.Action, Option[EventSource.Event]] = {
    case Create(items, uuid) if items.nonEmpty => Some(toPayload(items, uuid, "create-event"))
    case Update(items, uuid) if items.nonEmpty => Some(toPayload(items, uuid, "update-event"))
    case Delete(items, uuid) if items.nonEmpty => Some(toPayload(items, uuid, "delete-event"))
  }
}
