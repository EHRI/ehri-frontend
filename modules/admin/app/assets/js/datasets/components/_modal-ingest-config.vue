    ...
    submit: function () {
      ...
      this.api.saveImportConfig(
          this.dataset.id, {
            ...
            hierarchyFile: this.dataset.setHierarchy ? this.hierarchyFile : null,
  ...
  <modal-window v-on:close="$emit('close')">
    ...
    <template v-slot:footer>
      ...
      <button v-bind:disabled="!isValidConfig" v-on:click="submit"
        ...
        <template v-if="commit">{{ $t(`ingest.runIngest`) }}</template>
        <template v-else>{{ $t(`ingest.runDryRun`) }}</template>
      </button>
    </template>
  </modal-window>
</template>
