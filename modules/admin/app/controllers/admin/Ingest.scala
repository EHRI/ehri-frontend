...
import services.ingest.{FilePayload, IngestApi, IngestParams, LocalProperties}
      ...
      request.body.file(IngestParams.DATA_FILE).map { data =>
        boundForm.fold(
          ...
          ingestTask => {
            ...
            val task = ingestTask.copy(properties = LocalProperties(props), data = FilePayload(Some(data.ref)))
