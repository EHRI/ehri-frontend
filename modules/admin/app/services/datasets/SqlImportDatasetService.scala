...
case class SqlImportDatasetService @Inject()(db: Database, actorSystem: ActorSystem) extends ImportDatasetService {
  ...
  private implicit val parser: RowParser[ImportDataset] =
    Macro.parser[ImportDataset](
      "repo_id", "id", "name", "type", "content_type", "item_id", "sync", "nest", "set_hierarchy", "status", "comments", "created")
  ...
  override def create(repoId: String, info: ImportDatasetInfo): Future[ImportDataset] = Future {
    db.withConnection { implicit conn =>
      try {
        SQL"""INSERT INTO import_dataset (repo_id, id, name, type, content_type, item_id, sync, nest, set_hierarchy, status, comments)
          VALUES (
            $repoId,
            ${info.id},
            ${info.name},
    ...
    db.withConnection { implicit conn =>
      SQL"""UPDATE import_dataset
            SET
              ...
              sync = ${info.sync},
              nest = ${info.nest},
              set_hierarchy = ${info.setHierarchy},
              status = ${info.status},
    ...
    db.withTransaction { implicit conn =>
      val inserts = info.map { item =>
        Seq[NamedParameter](
          ...
          "nest" -> item.nest,
          "sync" -> item.sync,
          "set_hierarchy" -> item.setHierarchy,
          "status" -> item.status,
          "comments" -> item.notes
        )
      }
      val q = """INSERT INTO import_dataset (repo_id, id, name, type, content_type, item_id, nest, sync, set_hierarchy, status, comments)
                   VALUES({repo_id}, {id}, {name}, {type}, {content_type}, {item_id}, {nest}, {sync}, {set_hierarchy}, {status}, {comments})
