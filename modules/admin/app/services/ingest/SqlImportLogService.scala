...
case class SqlImportLogService @Inject()(db: Database, actorSystem: ActorSystem) extends ImportLogService {
  ...
  override def snapshots(repoId: String): Future[Seq[Snapshot]] = Future {
    db.withConnection { implicit conn =>
      SQL"""SELECT id, created, notes
           FROM repo_snapshot
           WHERE repo_id = $repoId
           ORDER BY created DESC""".as(snapshotParser.*)
    }
  }
      ...
      db.withTransaction { implicit conn =>
        val snapshot: Snapshot =
          SQL"""INSERT INTO repo_snapshot (repo_id, notes)
                VALUES ($repoId, $notes)
                RETURNING id, created, notes""".as(snapshotParser.single)
  ...
  override def cleanupInfo(repoId: String, snapshotId: Int): Future[Cleanup] = Future {
    db.withTransaction { implicit conn =>
      // Calculate the items that require cleanup. The snapshot, taken before
      // recent import operations, records all item IDs in a repository.
      // Import logs since the snapshot record all imported items, which we
      // assume are all items currently managed by the repository. Items that
      // were not touched by recent imports need removing.
      ...
      // in the touched set.) We use the last ID component (after the '-') as
      // a proxy for the local identifier, since we don't have that data in
      // the import log (it is a reliable proxy however.)
      //
      // Then select the untouched ID and the first touched item that has a local
      // identifier matching the local identifier of an untouched item. These will
      // be redirects. Everything untouched is a deletion.
      ...
      SQL"""INSERT INTO touched_items_#$ts
            ...
            FROM import_log log, import_file_mapping map, repo_snapshot snapshot
            WHERE map.import_log_id = log.id
              AND log.repo_id = $repoId
              AND snapshot.id = $snapshotId
              AND log.created > snapshot.created""".executeUpdate()
      ...
      // Indexes vital to making this operation manageable.
      ...
      SQL"""INSERT INTO untouched_items_#$ts (old_id, old_local)
            SELECT DISTINCT item.item_id AS old_id, reverse(split_part(reverse(item.item_id), '-', 1)) as old_local
            FROM repo_snapshot_item item
              JOIN repo_snapshot snapshot ON item.repo_snapshot_id = snapshot.id
  ...
  override def saveCommittedCleanup(repoId: String, snapshotId: Int, cleanup: Cleanup): Future[Int] = Future {
    db.withTransaction { implicit conn =>
  ...
  override def listCommittedCleanups(repoId: String, snapshotId: Int): Future[Seq[(Int, Instant)]] = Future {
    db.withConnection { implicit conn =>
  ...
  override def getCommittedCleanup(id: String, snapshotId: Int, cleanupId: Int): Future[Cleanup] = Future {
    db.withConnection { implicit conn =>
      val deletions =
        SQL"""SELECT item_id
              ...
              AND NOT EXISTS (
                SELECT DISTINCT map.item_id FROM import_file_mapping map
                  ...
                  WHERE log.repo_id = $repoId
                    ...
                    AND log.created > snapshot.created
              )
