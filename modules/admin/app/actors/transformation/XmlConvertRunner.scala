...
object XmlConvertRunner {
  ...
  case class Fetch(src: List[FileStage.Value], files: List[FileMeta], truncated: Boolean, last: Option[String], count: Int)
...
case class XmlConvertRunner (job: XmlConvertJob, transformer: XmlTransformer, storage: FileStorage)(
  ...
  override def receive: Receive = {
    ...
    case Initial =>
      ...
      msgTo ! Starting
      // TODO: Start!
      self ! Sources(job.data.config.src.toList, None)
  }
  ...
  def running(msgTo: ActorRef, done: Int, start: LocalDateTime): Receive = {
    ...
    case Sources(src :: rest, after) =>
      storage.listFiles(job.data.classifier, Some(job.data.inPrefix(src)), after, max = 200)
        .map(list => Fetch(src :: rest, list.files.toList, list.truncated, after, done))
        .pipeTo(self)
        ...
        .map {
          ...
          case Some((_, stream)) =>
            ...
            storage.putBytes(
              ...
              Fetch(src :: rest, others, truncated, Some(file.key), count + 1)
            }.pipeTo(self)
        }

    // Files in this batch exhausted, continue from last marker
    case Fetch(src :: rest, Nil, true, last, count) =>
    ...
    case Fetch(_ :: rest, Nil, false, _, count)  =>
      context.become(running(msgTo, count, start))
