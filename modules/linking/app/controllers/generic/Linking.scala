...
trait Linking[MT <: AnyModel] extends Read[MT] with Search {
  ...
  def linkSelectAction(id: String, toType: EntityType.Value, facets: FacetBuilder = emptyFacets)(
      f: MT => ItemPage[(AnyModel,SearchHit)] => SearchParams => List[AppliedFacet] => EntityType.Value => Option[UserProfile] => Request[AnyContent] => Result)(implicit rd: RestReadable[MT], rs: RestResource[MT], ct: RestContentType[MT]) = {
  ...
  def linkAction(id: String, toType: EntityType.Value, to: String)(f: MT => AnyModel => Option[UserProfile] => Request[AnyContent] => Result)(implicit rd: RestReadable[MT], rs: RestResource[MT], ct: RestContentType[MT]) = {
    withItemPermission.async[MT](id, PermissionType.Annotate) {
        item => implicit userOpt => implicit request =>
  ...
  def linkPostAction(id: String, toType: EntityType.Value, to: String)(
      f: Either[(MT, AnyModel,Form[LinkF]),Link] => Option[UserProfile] => Request[AnyContent] => Result)(implicit rd: RestReadable[MT], rs: RestResource[MT], ct: RestContentType[MT]) = {
  ...
  def linkMultiAction(id: String)(f: MT => Option[UserProfile] => Request[AnyContent] => Result)(implicit rd: RestReadable[MT], rs: RestResource[MT], ct: RestContentType[MT]) = {
  ...
  def linkPostMultiAction(id: String)(
      f: Either[(MT,Form[List[(String,LinkF,Option[String])]]),Seq[Link]] => Option[UserProfile] => Request[AnyContent] => Result)(implicit rd: RestReadable[MT], rs: RestResource[MT], ct: RestContentType[MT]): Action[AnyContent] = {
   ...
   */
  def createLink(id: String, apid: String)(implicit rd: RestReadable[MT], rs: RestResource[MT], ct: RestContentType[MT]) = {
   ...
   * other objects.
   */
  def createMultipleLinks(id: String)(implicit rd: RestReadable[MT], rs: RestResource[MT], ct: RestContentType[MT]) = {
    withItemPermission.async[JsValue, MT](parse.json, id, PermissionType.Annotate) {
        item => implicit userOpt => implicit request =>
      request.body.validate[List[AccessPointLink]].fold(
        ...
        anns => {
          ...
          backend.linkMultiple(id, links).map { newLinks =>
            Cache.remove(id)
            Created(Json.toJson(newLinks)(Writes.seq(Link.Converter.clientFormat)))
          }
        }
      )
    }
   ...
   * Create a link, via Json, for any arbitrary two objects, via an access point.
   */
  def createAccessPoint(id: String, did: String)(implicit rd: RestReadable[MT], rs: RestResource[MT], ct: RestContentType[MT]) = {
   ...
   */
  def deleteAccessPoint(id: String, did: String, accessPointId: String)(implicit rd: RestReadable[MT], rs: RestResource[MT], ct: RestContentType[MT]) = {
   ...
   * Delete a link.
   */
  def deleteLink(id: String, linkId: String)(implicit rd: RestReadable[MT], rs: RestResource[MT], ct: RestContentType[MT]) = {
    withItemPermission.async[MT](id, PermissionType.Annotate) {
        bool => implicit userOpt => implicit request =>
   ...
   */
  def deleteLinkAndAccessPoint(id: String, did: String, accessPointId: String, linkId: String)(implicit rd: RestReadable[MT], rs: RestResource[MT], ct: RestContentType[MT]) = {
    withItemPermission.async[MT](id, PermissionType.Annotate) {
        bool => implicit userOpt => implicit request =>
      for {
