...
case class PermissionDAO[T <: Accessor](val accessor: UserProfileRepr) extends RestDAO {
  ...
  def addGroup(groupId: String, etype: EntityType.Value, userId: String): Future[Either[RestError, Boolean]] = {
    WS.url(enc("%s/%s/%s/%s/%s".format(baseUrl,
      EntityType.Group, groupId, etype, userId)))
      .withHeaders(headers.toSeq: _*).post(Map[String, List[String]]()).map { response =>
        checkError(response).right.map(r => r.status == OK)
      }
  }
  
  def removeGroup(groupId: String, etype: EntityType.Value, userId: String): Future[Either[RestError, Boolean]] = {
    WS.url(enc("%s/%s/%s/%s/%s".format(baseUrl,
      EntityType.Group, groupId, etype, userId)))
