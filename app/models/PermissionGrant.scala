...
case class PermissionGrant(val e: Entity) extends WrappedEntity {
  ...
  val permission: PermissionType.Value = e.relations(PERM_REL).headOption.map(e => PermissionType.withName(e.id))
        .getOrElse(sys.error("Unexpected permission id in PermissionGrant data: " + e))
